#' Estimate a dependency network
#'
#' This function takes data and estimates the requested network.
#' The default network it estimates is a partial distance correlation (pdcor) network,
#' returning the saturated pdcor network as well as the 'selected' network by thresholding
#' at P < 0.05. Missing data is removed.
#'
...
#' @param zero.order should the zero order dcor also be computed? defaults to FALSE
...
#' @return zero.order.saturated.net = saturated zero order distance correlation network
#' @return zero.order.selected.net = selected zero order distance correlation network at given alpha
#'
#' @export


estimate.network <- function(data, select="sig", dcor.permutations=NULL, alpha=0.05, adjust = "fdr", adjust.threshold = 0.1, qp.threshold = 0.95, zero.order=FALSE) {
  ...
  if(zero.order == TRUE) {
    ...
    zero.order.saturated.net <- matrix(0, nrow=ncol(data), ncol=ncol(data))
    dimnames(zero.order.saturated.net) <- list(colnames(data), colnames(data))

    zero.order.selected.net <- matrix(0, nrow=ncol(data), ncol=ncol(data))
    dimnames(zero.order.selected.net) <- list(colnames(data), colnames(data))

    zero.order.net.p <- matrix(0, nrow=ncol(data), ncol=ncol(data))
    dimnames(zero.order.net.p) <- list(colnames(data), colnames(data))
  ...
  if(zero.order == TRUE) {
    ...
    #edge list - saves calculating each edge weight twice in symmetric matrix form
    zero.edge.set <- as.data.frame(matrix(0, nrow = edges, ncol = 4))
    colnames(zero.edge.set) <- c("edge", "dcor","P value",adjust)
    pairs <- combn(colnames(data), 2, FUN = function(x) paste(x, collapse = "-"), simplify = FALSE)
    pairs <- as.character(pairs)
    zero.edge.set$edge <- pairs
    zero.edge.set[,4] <- NA

    #check for missing data, remove and notify user
    if(any(is.na(data))) print("zero order dcors estimated by using pairwise.complete.obs method")

    d <- data

    # estimate pdcor and P value for each edge weight in edge list
    for (i in 1:nrow(zero.edge.set)) {

      #get first node pair
      node.pair <- strsplit(zero.edge.set[i,1], "-")
      v1 <- node.pair[[1]][1]
      v2 <- node.pair[[1]][2]
      ...
      if(select == "sig") {
        ...
        zero.edge.set$dcor[i] <- dc$estimate[2]
        zero.edge.set$`P value`[i] <- dc$p.value

      } else {

        dc <- energy::dcor(x,y)

        zero.edge.set$dcor[i] <- dc
        zero.edge.set$`P value`[i] <- NA
    ...
    #fill in saturated pdcor network matrix
    zero.order.saturated.net[lower.tri(zero.order.saturated.net)] <- zero.edge.set$dcor
    zero.order.saturated.net[upper.tri(zero.order.saturated.net)] <- t(zero.order.saturated.net)[upper.tri(zero.order.saturated.net)]
    dimnames(zero.order.saturated.net) <- list(colnames(data), colnames(data))
    ...
    if(adjust != "none") {
      ...
      zero.edge.set[,4] <- p.adjust(zero.edge.set$`P value`, method=adjust)

    }


    if(select == "sig" & all(is.na(zero.edge.set[,4]))) {

      #replace NA with 1 if no variability in binary node
      ind <- which(is.na(zero.edge.set$`P value`) == TRUE)
      zero.edge.set$`P value`[ind] <- 1

      zero.net.p <- matrix(0, nrow=nodes, ncol=nodes)

      zero.net.p[lower.tri(zero.net.p)] <- zero.edge.set$`P value`

      zero.net.p[upper.tri(zero.net.p)] <- t(zero.net.p)[upper.tri(zero.net.p)]

      #select edges based on alpha threshold
      non.sig.ind <- which(zero.net.p >= alpha)
      zero.order.selected.net <- zero.order.saturated.net
      zero.order.selected.net[non.sig.ind] <- 0
      ...
      if(select == "sig" & !all(is.na(zero.edge.set[,4]))) {
        ...
        adjust.p <- p.adjust(zero.net.p[lower.tri(zero.net.p)], method=adjust)

        zero.adjust.values <- matrix(0, nrow=nodes, ncol=nodes)
        zero.adjust.values[lower.tri(zero.adjust.values)] <- adjust.p
        zero.adjust.values[upper.tri(zero.adjust.values)] <- t(zero.adjust.values)[upper.tri(zero.adjust.values)]

        #select edges based on adjust threshold
        non.sig.ind <- which(zero.adjust.values >= adjust.threshold)
        zero.order.selected.net <- zero.order.saturated.net
        zero.order.selected.net[non.sig.ind] <- 0
  ...
  if(zero.order == TRUE) {
    ...
    return(list(saturated.net = saturated.net,
                ...
                structure = abs(sign(selected.net)),
                edge.set = edge.set,
                zero.order.saturated.net = zero.order.saturated.net,
                zero.order.selected.net = zero.order.selected.net,
                zero.order.edge.set = zero.edge.set,
                dcor.perms = dcor.permutations,
                alpha = alpha,
                select = select))
  }
